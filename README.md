...
Here we use the SHA256 key from a zipped folder which contains within it the experiment script and the pre-registration document. This SHA256 key can therefore not be generated until the pre-registration is written and the expreriment code is written.
...
WidFiles holds the subject identifiers after running GenerateWidFiles
...
The SHA256 key should be simply copied and pasted into the text input and the matching .json folder created in WidFiles selected with the file selector. It might be easier to select the file first.
...
To keep the provided files working, move the downloaded .csv file to the folder GeneratedTrials and make sure the name is like ExpX_trial_data.csv. Otherwise, the file paths in the following steps may need to be manually changed.
